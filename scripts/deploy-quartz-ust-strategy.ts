...
import { QUARTZ_UST_VAULT_ADDRESS } from "./data";
import {
...
const VAULT_ADDRESS = QUARTZ_UST_VAULT_ADDRESS;
...
async function main() {
  ...
  const StrategyQuartzUstLP = await ethers.getContractFactory(
    "StrategyQuartzUstLP"
  );
  //   address _want,
  //   uint256 _poolId,
  ...
  const strat = await StrategyQuartzUstLP.deploy(
    WANT,
    POOL_ID,
    CHEF_ADDRESS,
    VAULT_ADDRESS,
  ...
  console.log("StrategyQuartzUstLP deployed to:", strat.address);
}
